...
public final class PolicyMatrix {
    ...
    public static final Map<String, Map<String, Scope>> MATRIX = Map.ofEntries(
        ...
        // The department and doctor lists are a signpost, not clinical data: every signed-in
        // role needs them to drive a picker (the nurse's queue filter, the doctor list on the
        // booking page). Separated from slot.list so browsing slots stays a booking privilege.
        entry("directory.read",     Map.of("patient", ANY, "receptionist", ANY, "doctor", ANY, "nurse", ANY,
                                           "pharmacist", ANY, "lab_tech", ANY, "billing_clerk", ANY,
                                           "management", ANY, "sys_admin", ANY)),
        ...
        // The test catalogue carries no patient data, so it is role-level like slot.list.
        entry("lab.catalogue",     Map.of("doctor", ANY, "lab_tech", ANY, "sys_admin", ANY)),
        ...
        // The ward board lists a whole ward, so there is no single patient to scope a WARD
        // decision against. Authorisation is role-level and the SERVICE pins a nurse to their
        // own assigned_ward_id; per-patient reads off the board still go through WARD scope.
        entry("ward.roster",         Map.of("nurse", ANY, "doctor", ANY, "management", ANY, "sys_admin", ANY)),
