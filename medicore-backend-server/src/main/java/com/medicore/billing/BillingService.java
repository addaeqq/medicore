...
public class BillingService {
    ...
    public BillingService(InvoiceRepository invoices, InvoiceItemRepository items,
        ...
        this.gateway = gateway; this.jdbc = jdbc; this.audit = audit; this.em = em;
    }
    ...
    private Map<String, Object> verifyInternal(Payment p) {
        ...
        try {
            // fall back to our own payment id — that is the localReference sent at init
            vr = gateway.verifyStatus(p.getGatewayRef() != null
                ? p.getGatewayRef()
    ...
    private void refreshStatus(Invoice inv) {
        ...
        // total()/paidSum() query the tables directly, so items and payments persisted earlier
        // in this transaction must reach the database before the sums are computed.
        em.flush();
